...
st.title("⚡ Advanced Stock Delivery & Safe Dip Entry Dashboard (Zerodha Kite)")

# ==========================
# 1. SIDEBAR & CREDENTIALS
# ==========================
st.sidebar.header("🔑 Zerodha Kite Credentials")
api_key = st.sidebar.text_input("API Key", type="password")
access_token = st.sidebar.text_input("Access Token", type="password")

st.sidebar.header("🔍 Stock Search & Settings")
trading_symbol = st.sidebar.text_input("Enter NSE Trading Symbol", value="RELIANCE").upper()
...
if not api_key or not access_token:
    st.warning("⚠️ Please enter your Zerodha Kite API Key and Access Token in the sidebar to connect.")
    st.stop()
...
# ==========================
# 2. FETCH INSTRUMENT TOKEN
# ==========================
# Caching the instrument list so it doesn't download the massive CSV on every UI interaction
@st.cache_data(ttl=3600)
def get_instrument_token(api_key, access_token, symbol):
    k = KiteConnect(api_key=api_key)
    k.set_access_token(access_token)
    try:
        instruments = k.instruments(exchange=k.EXCHANGE_NSE)
...
with st.spinner("Connecting to Kite and fetching instrument details..."):
    instrument_token = get_instrument_token(api_key, access_token, trading_symbol)

if isinstance(instrument_token, str):
    st.error(f"Error fetching instruments from Kite: {instrument_token}")
    st.stop()
elif not instrument_token:
    st.error(f"Trading Symbol '{trading_symbol}' not found on NSE. Please check the spelling.")
    st.stop()

st.sidebar.success(f"Active Symbol: {trading_symbol} (Token: {instrument_token})")

# ==========================
# 3. FETCH HISTORICAL DATA
# ==========================
# Dynamically calculate the last 365 days for Moving Averages
to_date = datetime.datetime.now()
from_date = to_date - datetime.timedelta(days=365)

try:
    historical_records = kite.historical_data(
    ...
    st.error(f"Error fetching historical data from Kite: {e}")
    st.stop()

if not historical_records:
    st.error("No historical data available for this symbol in the selected timeframe.")
    st.stop()

# Convert to DataFrame and format to match existing code
full_data = pd.DataFrame(historical_records)
...
full_data.index = pd.to_datetime(full_data.index).tz_localize(None)

if len(full_data) < 20:
    st.error("Insufficient historical data available for technical analysis (minimum 20 days required).")
    st.stop()
...
rsi_score = 0
if 45 <= rsi <= 60: rsi_score = 20
elif 30 <= rsi < 45: rsi_score = 15
elif 60 < rsi <= 70: rsi_score = 10
elif rsi < 30: rsi_score = 15
elif 70 < rsi <= 80: rsi_score = -10
else: rsi_score = -25
...
if avg_1day_volatility_pct > 0:
    ...
    if ratio <= 0.6: feasibility_score = 25
    elif ratio <= 1.0: feasibility_score = 20
    elif ratio <= 1.5: feasibility_score = 10
    else: feasibility_score = 0

volume_score = 10 if current_vol >= vol_ma20 else 5

# Set news score statically since Kite does not supply news
news_score = 5
...
# ==========================
st.markdown(f"### Stock: **{trading_symbol}** | Horizon Target: **{selected_timeframe_label}**")
...
if current_price <= (safe_entry_price * 1.005):
    st.success(f"🎯 **PRICE IS AT A SAFE ENTRY POINT!** Current Market Price (₹{current_price:.2f}) is already close to or at the Safe Dip Entry level (₹{safe_entry_price:.2f}).")
else:
    st.info(f"💡 **WAIT FOR DIP OR PLACE LIMIT ORDER**: Current Price (₹{current_price:.2f}) is slightly higher than support. Placing a buy limit order near **₹{safe_entry_price:.2f}** gives a safer trade setup to guarantee a **₹1,000 profit**.")
...
if trading_days_count >= est_days_needed:
    st.success(f"✅ **SELECTED HORIZON IS SUFFICIENT!**\n\nYour active timeframe ({selected_timeframe_label}) is well suited. Based on daily volatility ({avg_1day_volatility_pct:.2f}%/day), the required **+{req_move_pct:.2f}%** surge for a **₹1,000 profit** is realistic within ~{est_days_needed} trading day(s).")
else:
    st.warning(f"⚠️ **RECOMMENDATION: INCREASE YOUR TIMEFRAME HORIZON!**\n\nReaching a ₹1,000 profit from CMP requires a **+{req_move_pct:.2f}%** move. Because this stock moves ~{avg_1day_volatility_pct:.2f}% per day, **{selected_timeframe_label} is too short**.\n\n👉 **Suggested Horizon**: Switch your dropdown to **{recommended_timeframe}** (~{est_days_needed} trading days) or enter at the **Safe Dip Price (₹{safe_entry_price:.2f})**.")
...
with right_col:
    ...
    st.info("ℹ️ News feeds are not provided natively by the Zerodha Kite API. Trade scores above have been calculated purely using price action, volume, and technical indicators.")
